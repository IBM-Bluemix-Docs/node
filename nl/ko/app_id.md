...
5. Passport를 설치하십시오. 명령행을 사용하여 Node.js 앱이 있는 디렉토리를 열고 다음 명령을 실행하십시오. 
  ...
  **참고**: 다른 프레임워크는 LoopBack과 같은 `Express` 프레임워크를 사용합니다. {{site.data.keyword.appid_short_notm}} 서버 SDK는 이러한 모든 프레임워크와 함께 사용할 수 있습니다. 
...
6. 나중에 SDK 초기화에 사용될 인증 정보 키 값을 찾으십시오. 
    ...
    * _**clientID**_, _**secret**_, _**oauth-server-url**_ - 이들 값은 서비스 대시보드의 **서비스 인증 정보** 탭에서 **인증 정보 보기**를 클릭하여 찾을 수 있습니다. 
...
2. express-session 미들웨어를 사용하도록 express 앱을 설정하십시오. **참고**: 프로덕션 환경에 적합한 세션 스토리지를 포함하는 미들웨어를 구성해야 합니다. 자세한 정보는 [expressjs 문서 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://github.com/expressjs/session){: new_window}를 참조하십시오.
    ```js
    var app = express();
    app.use(session({
        secret: "123456",
...
5. `server.js` 파일에 다음 코드를 추가하여 서비스 경로 재지정을 실행하십시오. 
  ...
  **참고**: 서비스는 다음 순서대로 경로 재지정합니다. 
  ...
  2. `passport.authenticate(name, {successRedirect: "...."})`에 지정되어 있는 대로 경로 재지정. 
  3. 애플리케이션 루트 디렉토리("/"). 
...
이 서비스는 권한 부여 프로세스를 맵핑하는 데 `OAuth 2` 권한 부여 유형을 사용합니다. Facebook과 같은 소셜 ID 제공자를 구성하는 경우에는 로그인 위젯을 호출하는 데 [Oauth2 권한 부여 플로우 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://oauthlib.readthedocs.io/en/stable/oauth2/grants/authcode.html){: new_window}가 사용됩니다. 자신의 고유 UI 페이지를 표시하는 경우에는 로그인하고 ID 토큰에 대한 액세스 권한을 얻는 데 [리소스 소유자 비밀번호 인증 정보 플로우 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](https://oauthlib.readthedocs.io/en/stable/oauth2/grants/password.html){: new_window}가 사용됩니다. 
...
3. 자신의 인증 정보로 [기본 구성](/docs/services/appid/identity-providers.html)을 업데이트하십시오. {{site.data.keyword.appid_short_notm}}에서는 서비스를 사용해 보는 데 사용할 수 있는 IBM 인증 정보를 제공합니다. 앱을 공개하기 전에는 이 구성을 업데이트해야 합니다. 
...
표시할 수 있는 페이지는 ID 제공자 구성에 따라 달라집니다. IBM은 사용자의 계정 정보에 액세스할 수 없으므로 이 서비스는 소셜 ID 제공자에 대한 고급 기능을 제공하지 않습니다. 사용자는 자신의 정보를 관리하려는 경우 해당 ID 제공자로 이동해야 합니다. 예를 들어, Facebook 비밀번호를 변경하려는 경우에는 www.facebook.com으로 이동해야 합니다. 
...
2. [디렉토리 및 메시지 설정](/docs/services/appid/cloud-directory.html)을 구성하십시오. 
...
사용자에게 사용자 정의된 경험을 제공하려 하십니까? 자신의 고유 UI 페이지를 표시하는 방법을 보려면 [{{site.data.keyword.appid_short_notm}} 문서](/docs/services/appid/login-widget.html#branding)를 참조하십시오.
{: tip}
...
4. 변경사항이 즉시 구현되었는지 확인하려면 1단계 및 2단계를 반복하십시오. 앱 코드를 업데이트할 필요는 없습니다. 
